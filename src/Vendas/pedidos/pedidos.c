...
int produtos_ped_list(GtkEntry *widget, GtkTreeView *treeview)
{
	...
	sprintf(query,"select c.razao, (SELECT DATE_FORMAT(o.dia, \"%%d/%%m/%%y\")), o.tipopag from orcamentos as o inner join terceiros as c on o.cliente = c.code where o.code = %s",entrada);
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
