struct _cfe_produto *get_cupom_itens_from_orc(int orc_code){
  ...
  sprintf(query,"select op.code, op.item, p.nome, ncm.cod_ncm, cst.charIdTrib, orig.idOrigem, u.sigla, op.unidades, op.valor_unit, op.desconto, ped.tipo_mov from Produto_Orcamento as op inner join produtos as p inner join unidades as u inner join pedidos as ped inner join ncm as ncm inner join cst_cson as cst inner join prod_origem as orig on op.produto = p.code and p.unidades  = u.code and ped.code = op.code and p.ncm = ncm.code and p.cst = cst.code and p.origem = orig.code where op.code = %i",orc_code);
  ...
  while((row = mysql_fetch_row(res))){
    ...
    cupom_itens[itens_qnt].CST = strdup(row[4]);

    cupom_itens[itens_qnt].Orig = strdup(row[5]);
    ...
    cupom_itens[itens_qnt].uCom = strdup(row[6]);

    sprintf(formatar_valor,"%010.4f",atof(row[7]));
    cupom_itens[itens_qnt].qCom = strdup(formatar_valor);

    sprintf(formatar_valor,"%010.2f",atof(row[8]));
    cupom_itens[itens_qnt].vUnCom = strdup(formatar_valor);

    cupom_itens[itens_qnt].indRegra = strdup("A");

    sprintf(formatar_valor,"%010.2f",atof(row[9]));
