...
int cotac_add_item(GtkWidget *button, int cod){;
  ...
  if(cotac_add_button[cod]){
    if(GTK_IS_WIDGET(cotac_add_button[cod])) gtk_widget_destroy(cotac_add_button[cod]);
	}
  ...
  cotac_prod_qnt_entry[cod][pos] = gtk_spin_button_new_with_range(1,100,1);
  ...
  cotac_prod_prc_entry[cod][pos] = gtk_spin_button_new_with_range(1,100,1);
  ...
  gtk_grid_attach (GTK_GRID(cotac_itens_grid), cotac_prod_frames[cod][pos],0,pos,1,1);

  #pragma GCC diagnostic ignored "-Wint-conversion"

  g_signal_connect(cotac_add_button[cod],"clicked",G_CALLBACK(cotac_add_item), pos);
