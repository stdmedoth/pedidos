...
void receber_subgrp_todos_code(GtkWidget *button, GtkTreeView *treeview)
{
...
int entry_subgrp_todos_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
...
int pesquisa_subgrp_todos(GtkWidget *widget, gpointer entry)
{
	enum {N_COLUMNS=3,COLUMN0=0, COLUMN1=1, COLUMN2=2};
	...
	psq_subgrp_todos_wnd = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(psq_subgrp_todos_wnd),3);
	gtk_window_set_keep_above(GTK_WINDOW(psq_subgrp_todos_wnd),TRUE);
	gtk_widget_set_size_request(psq_subgrp_todos_wnd,500,250);
	...
	gtk_container_set_border_width(GTK_CONTAINER(psq_subgrp_todos_wnd),10);
	...
	gtk_container_add(GTK_CONTAINER(psq_subgrp_todos_wnd),caixa_grande);
	g_signal_connect(treeview,"row-activated",G_CALLBACK(receber_psq_code_space),psq_subgrp_todos_wnd);
