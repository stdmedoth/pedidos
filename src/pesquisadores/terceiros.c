void receber_ter_code(GtkWidget *button, GtkTreeView *treeview)
{
...
int psq_ter(GtkWidget *button, GtkEntry *cod_ter_entry)
{
	enum {N_COLUMNS=5,COLUMN0=0, COLUMN1=1, COLUMN2=2, COLUMN3=3, COLUMN4=4};
	...
	psq_ter_wnd = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(psq_ter_wnd),3);
	gtk_window_set_icon_name(GTK_WINDOW(psq_ter_wnd),"system-search");
	gtk_window_set_keep_above(GTK_WINDOW(psq_ter_wnd),TRUE);
	gtk_widget_set_size_request(psq_ter_wnd,500,250);
	...
	g_signal_connect(treeview,"row-activated",G_CALLBACK(receber_psq_code_space),psq_ter_wnd);
