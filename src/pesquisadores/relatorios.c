...
int entry_cad_relat_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
	...
	sprintf(query,"select r.code, r.nome, t.nome from criador_relat as r inner join relat_tabelas_id as t on r.tabela = t.code where r.nome like '%c%s%c'",37,entrada,37);
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
...
int psq_cad_relat(GtkWidget *button, GtkEntry *cod_cad_relat_entry)
{
	...
	sprintf(query,"select r.code, r.nome, t.nome from criador_relat as r inner join relat_tabelas_id as t on r.tabela = t.code");
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
