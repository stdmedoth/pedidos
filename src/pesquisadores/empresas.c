void receber_empresa_code(GtkWidget *button, GtkTreeView *treeview)
{
	...
	gtk_widget_destroy(psq_empresa_wnd);
}

int entry_empresa_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
  ...
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeview),GTK_TREE_MODEL(treestore));
	...
	sprintf(query,"select code, cnpj, razao, cidade from empresa where nome like '%c%s%c'",37,entrada,37);
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
...
int psq_empresa(GtkWidget *button, GtkEntry *cod_empresa_entry)
{
	...
	psq_empresa_wnd = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(psq_empresa_wnd),3);
	gtk_window_set_title(GTK_WINDOW(psq_empresa_wnd),"Pesquisa Bancos");
	gtk_window_set_icon_name(GTK_WINDOW(psq_empresa_wnd),"system-search");
	gtk_window_set_keep_above(GTK_WINDOW(psq_empresa_wnd),TRUE);
	gtk_widget_set_size_request(psq_empresa_wnd,500,250);
	...
	gtk_tree_view_column_set_title(coluna2,"CNPJ");
	...
	gtk_tree_view_column_set_title(coluna3,"Razão");
	...
	sprintf(query,"select code, cnpj, razao, cidade from empresa");
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
	...
	gtk_container_set_border_width(GTK_CONTAINER(psq_empresa_wnd),10);
	...
	gtk_container_add(GTK_CONTAINER(psq_empresa_wnd),caixa_grande);
	g_signal_connect(pesquisa_entry,"activate",G_CALLBACK(entry_empresa_pesquisa),treeview);
	pesquisa_global_alvo = cod_empresa_entry;

	g_signal_connect(treeview,"row-activated",G_CALLBACK(receber_psq_code_space),psq_empresa_wnd);

	g_signal_connect(escolher_campo_button,"clicked",G_CALLBACK(receber_empresa_code),treeview);
	gtk_widget_show_all(psq_empresa_wnd);
	return 0;
}
