void receber_cotac_code(GtkWidget *button, GtkTreeView *treeview)
{
	...
	gtk_widget_destroy(psq_cotac_wnd);
}

int psq_cotac(GtkWidget *button, GtkEntry *cod_cotac_entry)
{
	enum {
		COLUMN0,
		COLUMN1,
	...
	psq_cotac_wnd = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(psq_cotac_wnd),3);
	gtk_window_set_icon_name(GTK_WINDOW(psq_cotac_wnd),"system-search");
	gtk_window_set_keep_above(GTK_WINDOW(psq_cotac_wnd),TRUE);
	gtk_widget_set_size_request(psq_cotac_wnd,500,250);
  ...
  gtk_tree_view_append_column(GTK_TREE_VIEW(treeview),coluna5);
	...
	sprintf(query,"select code, publica, DATE_FORMAT(data, '%%d/%%m/%%Y'), DATE_FORMAT(validade, '%%d/%%m/%%Y'), descricao from cotacoes order by  validade");
	res = consultar(query);
	if(!res){
		return 1;
	}
	...
	gtk_container_set_border_width(GTK_CONTAINER(psq_cotac_wnd),10);
	...
	gtk_container_add(GTK_CONTAINER(psq_cotac_wnd),caixa_grande);
	g_signal_connect(treeview,"row-activated",G_CALLBACK(receber_psq_code_space),psq_cotac_wnd);
	pesquisa_global_alvo = GTK_ENTRY(cod_cotac_entry);
	g_signal_connect(escolher_campo_button,"clicked",G_CALLBACK(receber_cotac_code),treeview);
	gtk_widget_show_all(psq_cotac_wnd);
	return 0;
}
