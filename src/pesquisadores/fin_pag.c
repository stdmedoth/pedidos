void pagar_fin_pag_code(GtkWidget *button, GtkTreeView *treeview)
{
	...
	gtk_widget_destroy(psq_fin_pag_wnd);
}

int entry_fin_pag_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
...
int psq_fin_pag(GtkWidget *button, GtkEntry *entry)
{
	enum {N_COLUMNS=7,COLUMN0=0, COLUMN1=1, COLUMN2=2, COLUMN3=3, COLUMN4=4, COLUMN5=5, COLUMN6=6};
	...
	psq_fin_pag_wnd = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(psq_fin_pag_wnd),3);
	gtk_window_set_icon_name(GTK_WINDOW(psq_fin_pag_wnd),"system-search");
	gtk_window_set_keep_above(GTK_WINDOW(psq_fin_pag_wnd),TRUE);
	gtk_widget_set_size_request(psq_fin_pag_wnd,500,250);
	...
	gtk_container_set_border_width(GTK_CONTAINER(psq_fin_pag_wnd),10);
	...
	gtk_container_add(GTK_CONTAINER(psq_fin_pag_wnd),caixa_grande);

	pesquisa_global_alvo = GTK_ENTRY(entry);

	g_signal_connect(treeview,"row-activated",G_CALLBACK(receber_psq_code_space),psq_fin_pag_wnd);
	g_signal_connect(pesquisa_entry,"activate",G_CALLBACK(entry_fin_pag_pesquisa),treeview);
	g_signal_connect(escolher_campo_button,"clicked",G_CALLBACK(pagar_fin_pag_code),treeview);

	gtk_widget_show_all(psq_fin_pag_wnd);
	return 0;
}
