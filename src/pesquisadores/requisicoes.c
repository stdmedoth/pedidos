void receber_req_code(GtkWidget *button, GtkTreeView *treeview)
{
	...
	gtk_widget_destroy(psq_req_wnd);
}

int entry_req_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
	...
	sprintf(query,"select r.code, p.nome, r.quantidade, DATE_FORMAT(r.data, '%%d/%%m/%%Y') from prod_requisicoes  as r inner join produtos as p on r.produto = p.code where p.nome like '%c%s%c' and r.status != %i", 37,entrada,37, REQ_PROD_CONCLUIDO);
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
...
int psq_req(GtkWidget *button, GtkEntry *cod_req_entry)
{
	...
	psq_req_wnd = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(psq_req_wnd),3);
	gtk_window_set_icon_name(GTK_WINDOW(psq_req_wnd),"system-search");
	gtk_window_set_keep_above(GTK_WINDOW(psq_req_wnd),TRUE);
	gtk_widget_set_size_request(psq_req_wnd,500,250);

	gtk_tree_view_column_pack_start(coluna1,celula1,TRUE);
	gtk_tree_view_column_set_title(coluna1,"Solicitação");
	...
	gtk_tree_view_column_set_title(coluna3,"Quantidade");
	...
	gtk_tree_view_column_set_title(coluna4,"Data");
	...
	sprintf(query,"select r.code, p.nome, r.quantidade, DATE_FORMAT(r.data, '%%d/%%m/%%Y') from prod_requisicoes as r inner join produtos as p on r.produto = p.code where r.status != %i", REQ_PROD_CONCLUIDO);
	res = consultar(query);
	if(res == NULL)
		return 1;
	...
	gtk_container_set_border_width(GTK_CONTAINER(psq_req_wnd),10);
	...
	gtk_container_add(GTK_CONTAINER(psq_req_wnd),caixa_grande);
	g_signal_connect(pesquisa_entry,"activate",G_CALLBACK(entry_req_pesquisa),treeview);
	pesquisa_global_alvo = cod_req_entry;

	g_signal_connect(treeview,"row-activated",G_CALLBACK(receber_psq_code_space),psq_req_wnd);

	g_signal_connect(escolher_campo_button,"clicked",G_CALLBACK(receber_req_code),treeview);
	gtk_widget_show_all(psq_req_wnd);
	return 0;
}
