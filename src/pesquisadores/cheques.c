void receber_cheq_code(GtkWidget *button, GtkTreeView *treeview)
{
	...
	gtk_widget_destroy(psq_cheq_wnd);
}

int entry_cheq_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
	...
	sprintf(query,"select code, conta, serie, numero from cheque where numero like '%c%s%c'",37,entrada,37);
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
...
int psq_cheq(GtkWidget *button, GtkEntry *cod_cheq_entry)
{
	...
	psq_cheq_wnd = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(psq_cheq_wnd),3);
	gtk_window_set_title(GTK_WINDOW(psq_cheq_wnd),"Pesquisa Chques");
	gtk_window_set_icon_name(GTK_WINDOW(psq_cheq_wnd),"system-search");
	gtk_window_set_keep_above(GTK_WINDOW(psq_cheq_wnd),TRUE);
	gtk_widget_set_size_request(psq_cheq_wnd,500,250);
	...
	gtk_tree_view_column_set_title(coluna2,"Conta");
	...
	gtk_tree_view_column_set_title(coluna3,"Série");
	...
	gtk_tree_view_column_set_title(coluna4,"Número");
	...
	sprintf(query,"select code, conta, serie, numero from cheque");
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
	...
	gtk_container_set_border_width(GTK_CONTAINER(psq_cheq_wnd),10);
	...
	gtk_container_add(GTK_CONTAINER(psq_cheq_wnd),caixa_grande);
	g_signal_connect(pesquisa_entry,"activate",G_CALLBACK(entry_cheq_pesquisa),treeview);
	pesquisa_global_alvo = cod_cheq_entry;

	g_signal_connect(treeview,"row-activated",G_CALLBACK(receber_psq_code_space),psq_cheq_wnd);

	g_signal_connect(escolher_campo_button,"clicked",G_CALLBACK(receber_cheq_code),treeview);
	gtk_widget_show_all(psq_cheq_wnd);
	return 0;
}
