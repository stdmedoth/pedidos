void receber_maildistrib_code(GtkWidget *button, GtkTreeView *treeview)
{
	...
	gtk_widget_destroy(psq_maildistrib_wnd);
}

void receber_maildistrib_code_space(GtkTreeView *treeview, GtkTreePath *path,  GtkTreeViewColumn *column, gpointer poinmaildistrib)
{
	GtkTreeSelection *selection;
	GtkTreeModel *model;
	GtkTreeIter iter;
...
int entry_maildistrib_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
  ...
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeview),GTK_TREE_MODEL(treestore));
	...
	sprintf(query,"select * from distribuicoes where nome like '%c%s%c' limit 20",37,entrada,37);
	res = consultar(query);
	if(res == NULL){
		return 1;
	}
	...
	{
		gchar *format_text = malloc(30);
		gtk_tree_store_append(modelo,&campos,NULL);
		g_print("Inserindo codigo: %s nome: %s\n",row[0],row[1]);
		int cont=0;

		sprintf(query,"select count(code) from PessoasDistribuicao where distribuicao = %s",row[0]);
		res2 = consultar(query);
		if(!res2){
			return 1;
		}
		if((row2 = mysql_fetch_row(res2))){
			sprintf(format_text, "%s Contatos", row2[0]);
		}
...
int psq_maildistrib(GtkWidget *button, GtkEntry *cod_maildistrib_entry)
{
	enum {N_COLUMNS=3,COLUMN0=0, COLUMN1=1, COLUMN2=2};
	...
	psq_maildistrib_wnd = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(psq_maildistrib_wnd),3);
	gtk_window_set_icon_name(GTK_WINDOW(psq_maildistrib_wnd),"system-search");
	gtk_window_set_keep_above(GTK_WINDOW(psq_maildistrib_wnd),TRUE);
	gtk_widget_set_size_request(psq_maildistrib_wnd,500,250);
	...
	gtk_tree_view_column_set_title(coluna3,"Contatos Qnt.");
	...
	sprintf(query,"select * from distribuicoes limit 20");
	...
	gtk_container_set_border_width(GTK_CONTAINER(psq_maildistrib_wnd),10);
	...
	gtk_container_add(GTK_CONTAINER(psq_maildistrib_wnd),caixa_grande);

	g_signal_connect(treeview,"row-activated",G_CALLBACK(receber_psq_code_space),psq_maildistrib_wnd);

	g_signal_connect(pesquisa_entry,"activate",G_CALLBACK(entry_maildistrib_pesquisa),treeview);

	pesquisa_global_alvo = GTK_ENTRY(cod_maildistrib_entry);

	gtk_widget_grab_focus(pesquisa_entry);
	g_signal_connect(escolher_campo_button,"clicked",G_CALLBACK(receber_maildistrib_code),treeview);
	gtk_widget_show_all(psq_maildistrib_wnd);
	return 0;
}
