...
int entry_prod_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
	...
	sprintf(query,"select p.code, p.nome, p.preco, p.peso, g.nome from produtos as p inner join grupos as g on g.code = p.grupo where p.nome like '%c%s%c'",37,entrada,37);
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
...
int psq_prod(GtkWidget *button, GtkEntry *cod_prod_entry)
{
	...
	sprintf(query,"select p.code, p.nome, p.preco, p.peso, g.nome from produtos as p inner join grupos as g on g.code = p.grupo");
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
