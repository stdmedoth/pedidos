void receber_cidd_code(GtkWidget *button, GtkTreeView *treeview)
{
	...
	gtk_widget_destroy(psq_cidd_wnd);
}

int entry_cidd_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
	...
	tipo_psq = gtk_combo_box_get_active(GTK_COMBO_BOX(psq_cidd_combo_box));
...
int psq_cidd(GtkWidget *button, GtkEntry *cod_cidd_entry)
{
	enum {N_COLUMNS=5,COLUMN0=0, COLUMN1=1, COLUMN2=2, COLUMN3=3, COLUMN4=4};
	...
	psq_cidd_combo_box = gtk_combo_box_text_new();
	gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(psq_cidd_combo_box),"0","Nome");
	gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(psq_cidd_combo_box),"1","UF");
	gtk_combo_box_text_append(GTK_COMBO_BOX_TEXT(psq_cidd_combo_box),"2","DDD");
	gtk_combo_box_set_active(GTK_COMBO_BOX(psq_cidd_combo_box),0);
	...
	psq_cidd_wnd = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(psq_cidd_wnd),3);
	gtk_window_set_icon_name(GTK_WINDOW(psq_cidd_wnd),"system-search");
	if(personalizacao.janela_keep_above==1)
		gtk_window_set_keep_above(GTK_WINDOW(psq_cidd_wnd),TRUE);
	gtk_widget_set_size_request(psq_cidd_wnd,500,250);
	...
	sprintf(query,"select id_cidade, descricao, uf, codigo_ibge, ddd from cidade");
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
	...
	gtk_box_pack_start(GTK_BOX(caixa_grande),psq_cidd_combo_box,0,0,0);
	gtk_box_pack_start(GTK_BOX(caixa_grande),pesquisa_entry,0,0,0);
	gtk_container_set_border_width(GTK_CONTAINER(psq_cidd_wnd),10);
	...
	gtk_container_add(GTK_CONTAINER(psq_cidd_wnd),caixa_grande);
	g_signal_connect(treeview,"row-activated",G_CALLBACK(receber_psq_code_space),psq_cidd_wnd);
	g_signal_connect(pesquisa_entry,"activate",G_CALLBACK(entry_cidd_pesquisa),treeview);
	pesquisa_global_alvo = GTK_ENTRY(cod_cidd_entry);
	g_signal_connect(escolher_campo_button,"clicked",G_CALLBACK(receber_cidd_code),treeview);
	gtk_widget_show_all(psq_cidd_wnd);
	return 0;
}
