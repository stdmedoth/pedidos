...
int entry_est_said_mov_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
	...
	sprintf(query,"select m.code, t.razao, m.saidas, DATE_FORMAT(m.data_mov, '%%d/%%m/%%Y') from movimento_estoque as m inner join terceiros as t on t.code = m.cliente where m.saidas > 0 and t.razao like '%c%s%c' limit 30",37,entrada,37);
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
...
int psq_est_said_mov(GtkWidget *button, GtkEntry *cod_est_said_mov_entry)
{
	...
	sprintf(query,"select m.code, t.razao, m.saidas, DATE_FORMAT(m.data_mov, '%%d/%%m/%%Y') from movimento_estoque as m inner join terceiros as t on t.code = m.cliente where m.saidas > 0 limit 30");
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
