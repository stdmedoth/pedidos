void receber_modelmail_code(GtkWidget *button, GtkTreeView *treeview)
{
	...
	gtk_widget_destroy(psq_modelmail_wnd);
}

void receber_modelmail_code_space(GtkTreeView *treeview, GtkTreePath *path,  GtkTreeViewColumn *column, gpointer poinmodelmail)
{
	GtkTreeSelection *selection;
	GtkTreeModel *model;
	GtkTreeIter iter;
...
int entry_modelmail_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
  ...
  gtk_tree_view_set_model(GTK_TREE_VIEW(treeview),GTK_TREE_MODEL(treestore));
	...
	sprintf(query,"select * from emails_model where nome like '%c%s%c' limit 20",37,entrada,37);
...
int psq_modelmail(GtkWidget *button, GtkEntry *cod_modelmail_entry)
{
	enum {N_COLUMNS=3,COLUMN0=0, COLUMN1=1, COLUMN2=2};
	...
	psq_modelmail_wnd = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(psq_modelmail_wnd),3);
	gtk_window_set_icon_name(GTK_WINDOW(psq_modelmail_wnd),"system-search");
	gtk_window_set_keep_above(GTK_WINDOW(psq_modelmail_wnd),TRUE);
	gtk_widget_set_size_request(psq_modelmail_wnd,500,250);
	...
	gtk_tree_view_column_set_title(coluna3,"Assunto");
	...
	sprintf(query,"select * from emails_model limit 20");
	...
	gtk_container_set_border_width(GTK_CONTAINER(psq_modelmail_wnd),10);
	...
	gtk_container_add(GTK_CONTAINER(psq_modelmail_wnd),caixa_grande);

	g_signal_connect(treeview,"row-activated",G_CALLBACK(receber_psq_code_space),psq_modelmail_wnd);

	g_signal_connect(pesquisa_entry,"activate",G_CALLBACK(entry_modelmail_pesquisa),treeview);

	pesquisa_global_alvo = GTK_ENTRY(cod_modelmail_entry);

	gtk_widget_grab_focus(pesquisa_entry);
	g_signal_connect(escolher_campo_button,"clicked",G_CALLBACK(receber_modelmail_code),treeview);
	gtk_widget_show_all(psq_modelmail_wnd);
	return 0;
}
