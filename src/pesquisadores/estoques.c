void receber_est_code(GtkWidget *button, GtkTreeView *treeview)
{
	...
	gtk_widget_destroy(psq_est_wnd);
}

int entry_est_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
	...
	sprintf(query,"select code, nome from estoques where nome like '%c%s%c'",37,entrada,37);
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
...
int psq_est(GtkWidget *button, GtkEntry *cod_est_entry)
{
	...
	psq_est_wnd = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(psq_est_wnd),3);
	gtk_window_set_icon_name(GTK_WINDOW(psq_est_wnd),"system-search");
	if(personalizacao.janela_keep_above==1)
		gtk_window_set_keep_above(GTK_WINDOW(psq_est_wnd),TRUE);
	gtk_widget_set_size_request(psq_est_wnd,500,250);
	...
	gtk_container_set_border_width(GTK_CONTAINER(psq_est_wnd),10);
	...
	gtk_container_add(GTK_CONTAINER(psq_est_wnd),caixa_grande);
	g_signal_connect(pesquisa_entry,"activate",G_CALLBACK(entry_est_pesquisa),treeview);
	pesquisa_global_alvo = GTK_ENTRY(cod_est_entry);

	g_signal_connect(treeview,"row-activated",G_CALLBACK(receber_psq_code_space),psq_est_wnd);

	g_signal_connect(escolher_campo_button,"clicked",G_CALLBACK(receber_est_code),treeview);
	gtk_widget_show_all(psq_est_wnd);
	return 0;
}
