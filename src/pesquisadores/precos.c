void receber_preco_code(GtkWidget *button, GtkTreeView *treeview)
{
	...
	gtk_widget_destroy(psq_preco_wnd);
}
...
int entry_preco_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
	...
	sprintf(query,"select prc.code, p.nome, prc.nome, prc.valor_fat, prc.valor_vist from precos as prc inner join produtos as p on prc.produto = p.code where p.nome like '%c%s%c' or prc.nome = '%c%s%c' order by p.code;",37,entrada,37,37,entrada,37);
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
	...
	{
    gchar *format_fat = malloc(30);
    gchar *format_vista = malloc(30);
    sprintf(format_fat, "R$ %.2f",atof(row[VLRFAT_COL]));
    sprintf(format_vista, "R$ %.2f",atof(row[VLRVISTA_COL]));
		...
		PRODNOME_COL,row[PRODNOME_COL],
		PRCNOME_COL,row[PRCNOME_COL],
		VLRFAT_COL,format_fat,
    VLRVISTA_COL,format_vista,
    -1);
	}
	return 0;
}

int psq_preco(GtkWidget *button, GtkEntry *cod_preco_entry)
{
  enum {
    CODE_COL,
    PRODNOME_COL,
  ...
  celula5 = gtk_cell_renderer_text_new();
	...
	psq_preco_wnd = gtk_window_new(GTK_WINDOW_TOPLEVEL);
	gtk_window_set_position(GTK_WINDOW(psq_preco_wnd),3);
	gtk_window_set_icon_name(GTK_WINDOW(psq_preco_wnd),"system-search");
  gtk_window_set_title(GTK_WINDOW(psq_preco_wnd),"Pesquisa Ordens");
	gtk_window_set_keep_above(GTK_WINDOW(psq_preco_wnd),TRUE);
	gtk_widget_set_size_request(psq_preco_wnd,500,250);
	...
	gtk_tree_view_column_add_attribute(coluna1,celula1,"text",CODE_COL);

	gtk_tree_view_column_pack_start(coluna2,celula2,TRUE);
	gtk_tree_view_column_set_title(coluna2,"Produto");
	gtk_tree_view_column_set_spacing(coluna2,5);
	gtk_tree_view_column_add_attribute(coluna2,celula2,"text",PRODNOME_COL);

	gtk_tree_view_column_pack_start(coluna3,celula3,TRUE);
	gtk_tree_view_column_set_title(coluna3,"Tabela");
	gtk_tree_view_column_set_spacing(coluna3,5);
	gtk_tree_view_column_add_attribute(coluna3,celula3,"text",PRCNOME_COL);

	gtk_tree_view_column_pack_start(coluna4,celula4,TRUE);
	gtk_tree_view_column_set_title(coluna4,"Valor Faturado");
	gtk_tree_view_column_set_spacing(coluna4,5);
	gtk_tree_view_column_add_attribute(coluna4,celula4,"text",VLRFAT_COL);

  gtk_tree_view_column_pack_start(coluna5,celula5,TRUE);
  gtk_tree_view_column_set_title(coluna5,"Valor A Vista");
  gtk_tree_view_column_set_spacing(coluna5,5);
  gtk_tree_view_column_add_attribute(coluna5,celula5,"text",VLRVISTA_COL);
  ...
  gtk_tree_view_append_column(GTK_TREE_VIEW(treeview),coluna5);
	...
	sprintf(query,"select prc.code, p.nome, prc.nome, prc.valor_fat, prc.valor_vist from precos as prc inner join produtos as p on prc.produto = p.code order by  p.code");
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
	...
	gtk_container_set_border_width(GTK_CONTAINER(psq_preco_wnd),10);
	...
	gtk_container_add(GTK_CONTAINER(psq_preco_wnd),caixa_grande);
	g_signal_connect(pesquisa_entry,"activate",G_CALLBACK(entry_preco_pesquisa),treeview);
	pesquisa_global_alvo = cod_preco_entry;

	g_signal_connect(treeview,"row-activated",G_CALLBACK(receber_psq_code_space),psq_preco_wnd);

	g_signal_connect(escolher_campo_button,"clicked",G_CALLBACK(receber_preco_code),treeview);
	gtk_widget_show_all(psq_preco_wnd);
	return 0;
}
