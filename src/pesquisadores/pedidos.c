...
int entry_ped_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
	...
	sprintf(query,"select o.code, c.razao, o.dia, o.total, o.observacoes from pediddos as inner o join terceiros as c on c.code = o.cliente where o.cliente like '%c%s%c'",37,entrada,37);
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
...
int psq_ped(GtkWidget *button, GtkEntry *cod_ped_entry)
{
	...
	gtk_tree_view_column_set_title(coluna5,"Observacões");
	...
	sprintf(query,"select o.code, c.razao, o.dia, o.total, o.observacoes from pedidos as o inner join terceiros as c on c.code = o.cliente");
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
