void est_said_confirmar_fun()
{
	...
	{
		sprintf(query,"insert into movimento_estoque( cliente, produto, estoque, pedido, subgrupo, saidas, tipo_mov, data_mov) values(%s,%s,%i, %s, %s,%s,%i,STR_TO_DATE('%s','%%d/%%m/%%Y'))",
	...
	{
		sprintf(query,"update movimento_estoque set cliente = %s, produto = %s, estoque = %i, pedido = %s, subgrupo = %s, saidas = %s, tipo_mov = %i, data_mov = STR_TO_DATE('%s','%%d/%%m/%%Y') where code = %s",
