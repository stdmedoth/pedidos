int est_sld_min_fun(){
  ...
  if((row = mysql_fetch_row(res))!=NULL){
    sprintf(query,"update saldo_min set saldo_min = '%s' where produto = %i and estoque = %i",
    est_sld_min_gchar,
    atoi(est_sld_prod_cod_gchar),
    est_sld_prod_est_int);
  }else{
    sprintf(query,"insert into saldo_min(produto,saldo_min,estoque) values(%i, %s, %i)",
    atoi(est_sld_prod_cod_gchar),
    est_sld_min_gchar,
    est_sld_prod_est_int);
  }
