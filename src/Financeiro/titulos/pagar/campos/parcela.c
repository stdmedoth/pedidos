int cad_pag_parcela_fun(){
  ...
  sprintf(query,"select tl.cliente, tl.pedido, tl.status, p.data_criacao, p.data_vencimento, p.valor from titulos as tl inner join terceiros as t inner join parcelas_tab as p on tl.code = p.parcelas_id and t.code = tl.cliente where tl.code = %i and p.posicao = %i and tl.tipo_titulo = 2",atoi(cad_pag_code_gchar), cad_pag_parcela_int);
