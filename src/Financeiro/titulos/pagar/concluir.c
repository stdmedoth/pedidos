int cad_pag_concluir_fun(){
  ...
  if(cad_pag_alterando == 0){
    ...
    if(cad_pag_parcela_int==0){
      sprintf(query,"insert into titulos(code,cliente,pedido,status, qnt_parcelas, tipo_titulo) values(%i,%i,%i,%i,0,2)",
    ...
    sprintf(query,"update titulos set cliente = %i, pedido = %i, status = %i, qnt_parcelas = 0, tipo_titulo = 2 where code = %i",
    atoi(cad_pag_cli_gchar),
    atoi(cad_pag_ped_gchar),
    cad_pag_status_int,
    atoi(cad_pag_code_gchar));
