int cad_rec_excluir_fun(){
  ...
  int titulo = atoi(cad_rec_code_gchar);
  ...
  if(PopupBinario(
    ...
    sprintf(query,"delete from parcelas_tab where parcelas_id = %i and posicao = %i",titulo,cad_rec_parcela_int);
    if(enviar_query(query)){
      popup(NULL,"Não foi possível deletar parcelas");
      return 1;
    }
