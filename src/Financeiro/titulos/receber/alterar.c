int cad_rec_alterar_fun(){
  ...
  gtk_combo_box_set_active(GTK_COMBO_BOX(cad_rec_status_combo),atoi(row[3]));
  gtk_entry_set_text(GTK_ENTRY(cad_rec_datacriacao_entry),row[4]);
  gtk_entry_set_text(GTK_ENTRY(cad_rec_datavencimento_entry),row[5]);
  gtk_entry_set_text(GTK_ENTRY(cad_rec_valor_entry),row[6]);
