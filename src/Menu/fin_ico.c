GtkWidget *fin_menu_get_icon_view(){
	...
	fin_icos_model = gtk_tree_store_new(N_COLUMNS,G_TYPE_STRING,GDK_TYPE_PIXBUF,G_TYPE_INT);

	gtk_tree_store_append(fin_icos_model,&iter,NULL);
	gtk_tree_store_set(fin_icos_model,
		&iter,
  ...
  gtk_tree_store_set(fin_icos_model,
	...
	gtk_tree_store_set(fin_icos_model,
		&iter,
		0,janelas_nomes[CAD_CHEQUES_WND],
		1,gtk_image_get_pixbuf(GTK_IMAGE(gtk_image_new_from_file(CHEQ_IMG))),
		2,CAD_CHEQUES_WND,-1);

  gtk_icon_view_set_model(GTK_ICON_VIEW(icon_view),GTK_TREE_MODEL(fin_icos_model));
	...
	//gtk_icon_view_set_activate_on_single_click(GTK_ICON_VIEW(icon_view),TRUE);
  	...
  	g_signal_connect(icon_view,"item-activated",G_CALLBACK(icon_view_select_caller), fin_icos_model);
