GtkWidget *tcn_menu_get_icon_view(){
	...
	tcn_icos_model = gtk_tree_store_new(N_COLUMNS,G_TYPE_STRING,GDK_TYPE_PIXBUF,G_TYPE_INT);

	gtk_tree_store_append(tcn_icos_model,&iter,NULL);
  gtk_tree_store_set(tcn_icos_model,
    &iter,
  ...
  gtk_icon_view_set_model(GTK_ICON_VIEW(icon_view),GTK_TREE_MODEL(tcn_icos_model));
  ...
  g_signal_connect(icon_view,"item-activated",G_CALLBACK(icon_view_select_caller), tcn_icos_model);
