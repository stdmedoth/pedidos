GtkWidget *cmp_menu_get_icon_view(){
  ...
  gtk_tree_store_set(modelo,
    &iter,0,janelas_nomes[REG_CAD_PROD],1,
    gtk_image_get_pixbuf(GTK_IMAGE(gtk_image_new_from_file(SOL_IMG))),
    2,REG_CAD_PROD,-1);

  gtk_tree_store_append(modelo,&iter,NULL);
  gtk_tree_store_set(modelo,
    &iter,0,janelas_nomes[REG_CAD_TER],1,
    gtk_image_get_pixbuf(GTK_IMAGE(gtk_image_new_from_file(COT_IMG))),
    2,REG_CAD_TER,-1);

  gtk_tree_store_append(modelo,&iter,NULL);
  gtk_tree_store_set(modelo,
    &iter,0,janelas_nomes[REG_CAD_OPER],1,
    gtk_image_get_pixbuf(GTK_IMAGE(gtk_image_new_from_file(ORD_IMG))),
    2,REG_CAD_OPER,-1);
