...
int entry_tables_relat_pesquisa(GtkEntry *widget, GtkTreeView *treeview)
{
	...
	sprintf(query,"select distinct(r.code), r.nome, t.nome from criador_relat as r inner join relat_tabelas_id as t on r.tabela = t.code where r.nome like '%c%s%c' and r.tabela = %i" ,37,entrada,37, tabelas_bd_id);
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
...
int psq_tables_relat(GtkWidget *button,  GtkEntry *cod_entry)
{
	enum {N_COLUMNS=3,COLUMN0=0, COLUMN1=1, COLUMN2=2};
	...
	sprintf(query,"select distinct(r.code), r.nome, t.nome from criador_relat as r inner join relat_tabelas_id as t on r.tabela = t.code where r.tabela = %i", tabelas_bd_id);
	res = consultar(query);
	if(res == NULL)
	{
		return 1;
	...
	pesquisa_global_alvo = GTK_ENTRY(cod_entry);
