int relat_sld_gerar_fun()
{
	...
	FILE *relatorio_file;
	int cont=0;
	gerando_file = malloc(strlen(MOV_RELAT_FILE)+16);
	sprintf(gerando_file,"%s%s.html",MOV_RELAT_FILE,randomize_string());
	...
	sprintf(query,"select b.nome from criador_relat as a inner join relat_tab_campos as b on a.campos = b.code where a.code = %s",relat_sld_codigo_gchar);
	...
	if((res2 = consultar(relat_sld_query_gchar))==NULL){
