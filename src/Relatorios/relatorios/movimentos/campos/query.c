int relat_sld_query_fun()
{
	...
	sprintf(query,"select b.query,t.inner_query from criador_relat as a inner join relat_tab_campos as b inner join relat_tabelas_id as t on a.campos = b.code and a.tabela = t.code where a.code = %s",relat_sld_codigo_gchar);
	...
	relat_sld_query_gchar = malloc(strlen(ini_query)+strlen(prod_query.campos)+strlen(end_query));
	sprintf(relat_sld_query_gchar,"%s%s%s",ini_query,prod_query.campos,end_query);
	gtk_entry_set_text(GTK_ENTRY(relat_sld_query_entry),relat_sld_query_gchar);
	return 0;
}
