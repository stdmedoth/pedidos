void relat_prod_fun()
{
	...
	gtk_entry_set_icon_from_icon_name(GTK_ENTRY(relat_prod_nome_entry),GTK_ENTRY_ICON_PRIMARY,"utilities-system-monitor");
	...
	gtk_fixed_put(GTK_FIXED(relat_prod_query_fixed),relat_prod_query_frame,20,80);
	...
	gtk_grid_attach(GTK_GRID(grid),relat_prod_query_fixed,0,2,1,1);
