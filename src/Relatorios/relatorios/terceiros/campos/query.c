int relat_ter_query_fun()
{
	...
	char end_query[] = " from terceiros as t";
	char query[MAX_QUERY_LEN];
	char campo_query_cp[MAX_QUERY_ELM_LEN];
	sprintf(query,"select b.query from criador_relat as a inner join relat_tab_campos as b on a.campos = b.code where a.code = %s",relat_ter_codigo_gchar);
	...
	while((row = mysql_fetch_row(res))!=NULL){
		...
		{
			sprintf(ter_query.campos," %s %s,",campo_query_cp,row[0]);//backup + campo novo
		}
		
		strcpy(campo_query_cp,ter_query.campos); //pega backup query
