...
create table terceiros ( code int primary key auto_increment not null, razao varchar(300) not null, doc varchar(20), tipo_doc int not null, tipoc varchar(20), tipo int, cep varchar(15) , endereco varchar(400), cidade varchar(50),uf varchar(20),numrua int, tiporua int, telefone varchar(15), contatot varchar(15), celular varchar(15), contatoc varchar(15), email varchar(100), contatoe varchar(20), prazo varchar(20), obs varchar(500));
...
create table produtos( code int primary key auto_increment not null, nome varchar(150) not null, preco float not null, peso float not null, unidade int not null, fornecedor int not null, grupo int not null, fator int , observacoes varchar(500) not null, foreign key(unidade) references unidades(code), foreign key(fornecedor) references terceiros(code), foreign key(grupo) references grupos(code));
...
create table Produto_Orcamento( code int not null,item int not null, produto int not null, unidades float not null, valor_unit float not null, tipodesc int, desconto float, total float not null,foreign key(code) references orcamentos(code));
